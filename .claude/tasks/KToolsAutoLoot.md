...
#### Панель фильтра / добавления (над списком)
`[checkbox] [ID input] [Название предмета input] [>= ilvl input] [checkbox]`
Первый checkbox — фильтрация по состоянию «включено». Последний checkbox — применить/добавить запись.
#### Колонки списка
